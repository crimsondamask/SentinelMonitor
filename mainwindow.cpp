...
Qt::ItemFlags SentinelTableModel::flags(const QModelIndex &index) const { return QAbstractTableModel::flags(index) & ~Qt::ItemIsUserCheckable; }
QVariant      SentinelTableModel::headerData(int section, Qt::Orientation orientation, int role) const {
...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(linksList, &QComboBox::currentIndexChanged, this, &MainWindow::selectedLinkChanged);
    ...
    connect(tableView, &QTableView::doubleClicked, this, &MainWindow::tagRowClicked);
    for (size_t i = 0; i < N_CHANNELS; i++) {
...
void    MainWindow::postWriteRequest(int linkId, int tagId, SentinelTagValue value) {
       ...
       connect(this->writeTagReply.get(), &QNetworkReply::finished, this, &MainWindow::writeTagFinished);
}

void MainWindow::writeTagFinished() {
    if (this->writeTagReply->error() == QNetworkReply::NetworkError::NoError) {
        int statusCode = writeTagReply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
        if (statusCode == 200) {
            QMessageBox::information(this, "Success", "Tag write was successfull.");
        } else {
            QMessageBox::critical(this, "Server Error", "The server could not write the tag successfully.");
        }
    } else {
        QMessageBox::critical(this, "Write Error", "Could not write the tag successfully.");
    }
}
...
void MainWindow::tagRowClicked(const QModelIndex &index) {
    ...
    if (this->linksBuffer[selectedLinkIndex].type == ST_DEVICE) {
        SentinelDeviceTag selectedTag = this->linksBuffer[selectedLinkIndex].deviceLink.tags[clickedRow];

        if (clickedColumn == 2) {
            switch (selectedTag.value.type) {
                case ST_INT_VALUE:
                    intInput = QInputDialog::getInt(this, QString("%1:%2 INT Input").arg(selectedTag.displayTk(), selectedTag.displayName()), tr("Write INT Value:"), selectedTag.value.int_value,
                                                    INT_MIN, INT_MAX, 1, &ok);
                    if (ok) {
                        tagValue = SentinelTagValue{.type = ST_INT_VALUE, .real_value = 0.0, .int_value = intInput, .bit_value = 0};
                    }
                    break;
                case ST_REAL_VALUE:

                    realInput = QInputDialog::getDouble(this, QString("%1:%2 REAL Input").arg(selectedTag.displayTk(), selectedTag.displayName()), tr("Write REAL Value:"),
                                                        selectedTag.value.real_value, FLT_MIN, FLT_MAX, 5, &ok);
                    if (ok) {
                        tagValue = SentinelTagValue{.type = ST_REAL_VALUE, .real_value = realInput, .int_value = 0, .bit_value = 0};
                    }
                    break;
                case ST_BIT_VALUE:
                    bitInput = QInputDialog::getInt(this, QString("%1:%2 Bit Input").arg(selectedTag.displayTk(), selectedTag.displayName()), tr("Write BIT Value:"), selectedTag.value.bit_value, 0, 1,
                                                    1, &ok);
                    if (ok) {
                        tagValue = SentinelTagValue{.type = ST_BIT_VALUE, .real_value = 0.0, .int_value = 0, .bit_value = bitInput};
                    }
                    break;
                default:
                    break;
            ...
            int enabled = {};
            enabled     = QInputDialog::getInt(this, QString("%1:%2 Enable Tag").arg(selectedTag.displayTk(), selectedTag.displayName()), tr("Enable Tag:"), (int)selectedTag.enabled, 0, 1, 1, &ok);
            if (ok) {
        ...
        SentinelInputsTag selectedTag = this->linksBuffer[selectedLinkIndex].inputsLink.tags[clickedRow];

        if (clickedColumn == 2) {
            switch (selectedTag.value.type) {
            case ST_INT_VALUE:
                intInput = QInputDialog::getInt(this, QString("%1:%2 INT Input").arg(selectedTag.displayTk(), selectedTag.displayName()), tr("Write INT Value:"), selectedTag.value.int_value, INT_MIN,
                                                INT_MAX, 1, &ok);
                if (ok) {
                    tagValue = SentinelTagValue{.type = ST_INT_VALUE, .real_value = 0.0, .int_value = intInput, .bit_value = 0};
                }
            ...
            case ST_BIT_VALUE:
                bitInput =
                    QInputDialog::getInt(this, QString("%1:%2 Bit Input").arg(selectedTag.displayTk(), selectedTag.displayName()), tr("Write BIT Value:"), selectedTag.value.bit_value, 0, 1, 1, &ok);
                if (ok) {
                    tagValue = SentinelTagValue{.type = ST_BIT_VALUE, .real_value = 0.0, .int_value = 0, .bit_value = bitInput};
                }
                break;
        ...
        QMessageBox::information(nullptr, "Unimplemented Link", "Selected link doesn't accept writing.");
        return;
    }
...
void MainWindow::saveActionClicked() {
    if (this->isError()) {
        ...
        QString fileName = QFileDialog::getSaveFileName(this, tr("Save Config"), QDir::homePath(), tr("Text Files (*.json);;All Files (*)"));
        ...
        if (!fileName.isEmpty()) {
            ...
            if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
                ...
                if (file.commit()) {
                    QMessageBox::information(this, tr("Success"), tr("Configuration file saved successfully."));
                } else {
...
void MainWindow::selectedLinkChanged() {
    ...
    if (this->linksList->currentIndex() > arrayLen - 1) {
        ...
        this->statusLabel->setText(QString("Selected link index exceeds links array bounds."));
    } else {
...
void MainWindow::initRequest() {
    ...
    connect(reply.get(), &QNetworkReply::finished, this, &MainWindow::parseServerData);
}

void MainWindow::parseServerData() {
    if (reply->error() != QNetworkReply::NoError) {
        this->error       = true;
        this->serverError = QString("Error connecting to server. %1").arg(reply->errorString());
    ...
    for (size_t i = 0; i < arrayLen; i++) {
        ...
        if (value.toObject().value("Device").isObject()) {
            QJsonObject deviceLinkObject = value.toObject().value("Device").toObject();
            ...
            for (size_t tag_index = 0; tag_index < N_CHANNELS; tag_index++) {
                ...
                if (!tagObject.value("address").isObject()) {
                    this->error       = true;
                    this->serverError = QString("tag %1 address parse failed.").arg(tag_index);
                    this->statusLabel->setText(this->serverError);
                    return;
                }
                ...
                if (tagAddressObject.value("ModbusAddr").isObject()) {
                    QJsonObject modbusAddrObject = tagAddressObject.value("ModbusAddr").toObject();
                    if (!modbusAddrObject.value("Holding").isDouble()) {
                        this->error       = true;
                        this->serverError = QString("tag %1 modbus address parse failed.").arg(tag_index);
                        this->statusLabel->setText(this->serverError);
                        return;
                    }
                    ...
                    deviceLink.tags[tag_index].tagDetails             = QString("%1,%2,Modbus,Holding=%3").arg(tagTk, tagName).arg(holdingRegister);

                } else {
                    this->error       = true;
                    this->serverError = QString("tag %1 address object is not implemented.").arg(tag_index);
                    this->statusLabel->setText(this->serverError);
                    return;
                }
            ...
            QString lastPollString    = deviceLinkObject.value("last_poll_time").toString();
            ...
            QJsonObject protocolObject = deviceLinkObject.value("protocol").toObject();

            if (protocolObject.value("ModbusTcp").isObject()) {
                QJsonObject modbusTcpObject = protocolObject.value("ModbusTcp").toObject();
                ...
                deviceLink.protocolDetails = QString("ModbusTcp:%1:%2").arg(ipValue).arg(portValue);

                SentinelModbusTcp modbusTcpConfig = SentinelModbusTcp(ipValue, portValue);

                deviceLink.setConfig(modbusTcpConfig);
            } else if (protocolObject.value("ModbusSerial").isObject()) {
                QJsonObject modbusSerialObject = protocolObject.value("ModbusSerial").toObject();

                if (!modbusSerialObject.value("ip").isString()) {
                    this->error       = true;
                    this->serverError = QString("Could not parse ModbusSerial com port value.");
                    this->statusLabel->setText(this->serverError);
                    return;
                }
                ...
                if (!modbusSerialObject.value("baudrate").isDouble()) {
                    this->error       = true;
                    this->serverError = QString("Could not parse ModbusSerial baudrate value.");
                    this->statusLabel->setText(this->serverError);
                    return;
                }
                ...
                if (!modbusSerialObject.value("slave").isDouble()) {
                    this->error       = true;
                    this->serverError = QString("Could not parse ModbusSerial slave value.");
                    this->statusLabel->setText(this->serverError);
                    return;
                }

                int                  slaveValue         = modbusSerialObject.value("slave").toInt();
                SentinelModbusSerial modbusSerialConfig = SentinelModbusSerial(comPortValue, baudrateValue, slaveValue);

                deviceLink.protocolDetails = QString("ModbusSerial:%1:Baudrate(%2):Slave(%3)").arg(comPortValue).arg(baudrateValue).arg(slaveValue);
...
void MainWindow::updateView() {
    ...
    if (this->selectedLinkIndex > arrayLen - 1) {
        this->error       = true;
        this->serverError = QString("Selected link index is outside the bounds of the links list.");
        this->statusLabel->setText(this->serverError);
...
void MainWindow::reconfigTagFinished() {
    ...
    if (this->reconfigTagReply->error() == QNetworkReply::NetworkError::NoError) {
        int statusCode = reconfigTagReply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
        if (statusCode == 200) {
            QMessageBox::information(this, "Success", "Tag reconfigured successfull.");
        } else {
            QMessageBox::critical(this, "Server Error", "The server could not reconfigure the tag successfully.");
        }
    } else {
        QString errorMsg = QString("Could not reconfigure tag. %1").arg(reconfigTagReply->errorString());
        QMessageBox::critical(this, "Reconfigure Error", errorMsg);
    }
}
