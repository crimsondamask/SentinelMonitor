...
SentinelDeviceTag::SentinelDeviceTag(qint16 id, QString tk) {
    ...
    this->value   = SentinelTagValue{.type = 0, .real_value = 0.0, .int_value = 0, .bit_value = 0};
...
SentinelDeviceLink::SentinelDeviceLink(qint16 id, QString tk, SentinelConfig config) : config(config) {
    this->id       = id;
    this->tk       = tk;
...
SentinelDeviceLink::SentinelDeviceLink(qint16 id, QString tk) : config(SentinelModbusTcp("192.168.0.1", 502)) {
    this->id       = id;
    this->tk       = tk;
    this->name     = QString("%1%2").arg(tk).arg(id);
    this->enable   = true;
...
SentinelConfig::SentinelConfig(SentinelModbusTcp config) : modbusTcp(config), modbusSerial("COM1", 9600, 1) {}
SentinelConfig::SentinelConfig(SentinelModbusSerial config) : modbusTcp("192.168.0.1", 502), modbusSerial(config) {}

SentinelModbusTcp::SentinelModbusTcp(QString ip, int port) : ip(ip), port(port) {}

SentinelModbusSerial::SentinelModbusSerial(QString comPort, int baudrate, int slave, int parity) : comPort(comPort), baudrate(baudrate), slave(slave), parity(parity) {}
SentinelModbusSerial::SentinelModbusSerial(QString comPort, int baudrate, int slave) : comPort(comPort), baudrate(baudrate) {}
